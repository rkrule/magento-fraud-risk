...
class Radial_Dom_Document extends DOMDocument
{
	...
	/**
	 * Create and attach a Radial_Dom_Element node with the
	 ...
	 * @return Radial_Dom_Document This document
	 */
	public function addElement($name, $val=null, $nsUri='')
	{
		...
		$el = $this->appendChild(new Radial_Dom_Element($name, '', $nsUri));
		if (!is_null($val)) {
			$el->appendChild(Radial_Dom_Helper::coerceValue($val));
		}
		return $this;
	}
	 ...
	 *
	 * @see self::addElement
	 * @return Radial_Dom_Element The created node.
	 */
	public function createElement($name, $val=null, $nsUri='')
	{
		$el = new Radial_Dom_Element($name, '', $nsUri);
		if (!is_null($val)) {
	 ...
	 * @return Radial_Dom_Element The last element specified in the path
	 */
	public function setNode($path, $value=null, DOMNode $contextNode=null, $nsUri='')
	{
		if (!$path) {
			if ($value && $contextNode) {
				$contextNode->appendChild(Radial_Dom_Helper::coerceValue($value));
			}
			return $contextNode;
		}
