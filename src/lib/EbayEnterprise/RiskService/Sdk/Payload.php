...
abstract class EbayEnterprise_RiskService_Sdk_Payload
	implements EbayEnterprise_RiskService_Sdk_IPayload
{
	/** @var EbayEnterprise_RiskService_Sdk_Xsd_Validator */
	...
	protected function _deserializeSubpayloadExtractionPaths(DOMXPath $xpath)
	{
		foreach ($this->_subpayloadExtractionPaths as $setter => $path) {
			...
			if ($foundNode && $this->$getter() instanceof EbayEnterprise_RiskService_Sdk_IPayload) {
	 ...
	 *
	 * @return EbayEnterprise_RiskService_Sdk_Xsd_Validator
	 */
	protected function _getXsdValidator()
	{
		if (!$this->_schemaValidator) {
			$this->_schemaValidator = $this->_buildPayloadForModel('EbayEnterprise_RiskService_Sdk_Xsd_Validator');
		}
	...
	/**
	 * Build a new EbayEnterprise_RiskService_Sdk_IPayload for the given interface.
	 *
	 * @param  strings
